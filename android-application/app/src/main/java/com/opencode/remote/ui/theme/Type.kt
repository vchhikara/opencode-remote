...
// Full M3 type scale, deliberately defined (not the bare `Typography()` default)
// so screen titles/labels read as a designed scale rather than stock values.
// Stays on the system default font (no bundled font asset needed) but bumps
// weight on the styles that carry the most visual weight in this app —
// TopAppBar titles (titleLarge), card/section headers (titleMedium/Small),
// and the AgentStateBadge label (labelSmall) — for more contrast against body text.
val AppTypography = Typography(
    ...
    labelSmall = TextStyle(fontFamily = FontFamily.Default, fontWeight = FontWeight.Bold, fontSize = 11.sp, lineHeight = 16.sp, letterSpacing = 0.5.sp)
)
