...
Both commands are verified to print `BUILD SUCCESSFUL` in this project's own CI/merge gates (see `PROGRESS.md`). Open the `android-application/` directory in Android Studio to run on a device or emulator, or install the assembled APK from `app/build/outputs/apk/debug/`.
...
See [bridge/API.md](../bridge/API.md) for the full WebSocket protocol and [ADR 0001](../adr/0001-per-device-tokens.md) for why per-device tokens exist.

## Repository layout

- `bridge/` — the Node.js WebSocket bridge (`main.js`) and its tests
- `android-application/` — the Android client (Kotlin/Compose)
- `adr/` — architecture decision records
- `.docs/` — project documentation (this folder); see below for a map

## Documentation map

- [PROGRESS.md](PROGRESS.md) — the ledger of what's actually been built and verified, phase by phase. Read this first for current project state.
- [IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md) — the detailed phased plan `PROGRESS.md` tracks against.
- [remote-control-roadmap.md](remote-control-roadmap.md) — the original feature roadmap that `IMPLEMENTATION_PLAN.md` was derived from.
- [reports/](reports/) — a dated review snapshot (code review, spec conformance, docs audit, security audit) from 2026-08-25.
- [archive/](archive/) — superseded plans and design docs, kept for history only; not current.
